...
int main(int argc, char *argv[]){
    ...
    std::vector<int> start_times(N), end_times(N), categories(N), L(M);
    ...
    //std::fill(dp.begin(), dp.begin() + M + 1, 0);
        ...
        for (int j = 1; j <= M; j++) {
            ...
            for (int k = 0; k < i; k++) {
                if (categories[k] == j && end_times[k] <= start_times[i] && dp[(k*(M+1)) + j-1] + 1 <= L[j-1]) {
                    max_count = std::max(max_count, dp[(k*(M+1)) + j-1] + 1);
                } else {
                    max_count = std::max(max_count, dp[(k*(M+1)) + j]);
                }
            }
    ...
    int max_count = 0;
    for (int j = 1; j <= M; j++) {
    max_count = std::max(max_count, dp[(N*(M+1)) + j]);
    }
    ...
    // std::ofstream file;
    // file.open ("./../resultados/"+arquivo+".csv", std::ios_base::app);
    // file << "," << std::to_string(N) << "," << std::to_string(M) << "," << "0" << "," << std::to_string(duration_cast<milliseconds>(te - ts)) << "," << std::to_string(max_count) << std::endl;
    // file.close();
}
